...
test("ホーム → レベル一覧: 1 が次に遊ぶ面、2 は未解放", async ({ page }) => {
  ...
  await expect(page.getByTestId("trays")).toHaveText("6");
});
...
test("トレイを使い切ると失敗し、理由を出してやり直せる", async ({ page }) => {
  const state = makeState({
    mode: "level",
  ...
  await expect(overlay).toBeVisible({ timeout: 10_000 });
  await expect(page.getByTestId("level-fail-reason")).toHaveText("Out of trays");
...
test("素焼きの欠片を描き、読み上げでも区別する", async ({ page }) => {
  ...
  board[0] = 7;
  const state = makeState({ mode: "level", board, level: { no: 1, goal: 3, trayLimit: 6 } });
  await gotoState(page, state, "/level?n=1");
