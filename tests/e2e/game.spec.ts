...
test("smoke: ホーム → エンドレス → ピースを置くとスコアが増える", async ({ page }, info) => {
  ...
  // 開始盤はシードでタイルが散らばるので、置ける場所は実行時に決める(docs/01 §4.2)。
...
test("clear: 1 列そろうと消えて空セルに戻る", async ({ page }) => {
  await gotoState(
    page,
    makeState({ board: almostFullRow(), piece: { cells: [[0, 0]], color: 2 }, score: 100 }),
    "/play",
  );
...
test("gameover: 詰みの盤で置くとオーバーレイ → もう一度で新しいゲーム", async ({ page }) => {
  await gotoState(
    ...
    makeState({
      board: almostDead(),
      piece: { cells: [[0, 0]], color: 2 },
      heat: 2, // 次のかけらは 2 マス → 飛び飛びの空きには置けない
      score: 4200,
      linesCleared: 18,
      longestStreak: 5,
      moves: 52,
...
test("resume: 数手置いてリロード → 続きから で盤が復元する", async ({ page }) => {
  ...
  await placeNext(page); // 始めの盤にはタイルが散っているので、置ける場所に置く
  await expect(page.getByTestId("score")).not.toHaveText("0");
...
test("keyboard: キーボードだけで 1 ピース置ける(docs/01 §8.2)", async ({ page }) => {
  await gotoState(page, makeState({ piece: { cells: [[0, 0]], color: 2 } }), "/play");

  // 逆手は手持ちが 1 個なので「選ぶ」操作が無い。矢印でカーソルが出る(docs/01 §8.2)
...
test("keyboard: Esc でカーソルを消すとゴーストが消える", async ({ page }) => {
  await gotoState(page, makeState({ piece: { cells: [[0, 0]], color: 2 } }), "/play");
  await page.keyboard.press("ArrowRight");
...
test("盤外にドロップするとキャンセルされる(docs/01 §8.1)", async ({ page }) => {
  ...
  const slot = await page.getByTestId("hand").boundingBox();
...
test("効果音: 置いた時と消えた時に音が出る(docs/10 §6)", async ({ page }) => {
  ...
  await gotoState(page, makeState({ board, piece: { cells: [[0, 0]], color: 2 } }), "/play");
  await waitForBoardLayout(page);
  ...
  const cleared = await nodes();

  // 消えない手 → 置く音(短い単発)。消える音より音数が少ない
  await reset();
  await placeNext(page);
  await expect.poll(nodes, { timeout: 15_000 }).toBeGreaterThan(0);
  expect(await nodes()).toBeLessThan(cleared);
});
