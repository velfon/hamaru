...
10×10 の盤にかけらを置き、列を揃えて消すパズル。**https://hamaru.lovenf.workers.dev**

供給に運はない。**次のかけらは、いま置いた場所のまわりの盤の形がそのまま出てくる**(「逆手」)。
列を消さずにいると「熱」が上がり、出てくるかけらが 1 マスずつ大きくなる。ルールは [docs/01](docs/01-game-spec.md) §1。
